...
public class InstrumentationUtils {
    ...
    public static String getMethodId(String className, Method method) {
        return String.format("%s.%s", className,
            getMethodWithSignature(method.getName(), method.getSignature()));
    }
    ...
    public static ClassLocation getClassLocation(String targetMethod) {
        ...
        if (lastFrag.contains("(")) {
            ...
            return new ClassLocation(targetMethod.substring(0, methodNameIdx),
                targetMethod.substring(methodNameIdx + 1, idx), lineNo);
        }
    }

    public static String toTargetMethodId(ClassLocation targetMethod) {
        if (targetMethod.getLineNumber() >= 0) {
            return String.format("%s.%s.%d", targetMethod.getClassCanonicalName(),
                targetMethod.getMethodSign(),
                targetMethod.getLineNumber());
        } else {
            return String.format("%s.%s", targetMethod.getClassCanonicalName(),
                targetMethod.getMethodSign());
        }
    }
