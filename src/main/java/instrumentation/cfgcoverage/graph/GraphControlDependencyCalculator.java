...
/**
 * @author lyly Adopt code in Yun Lin's CFGConstructor, just parameterize Graph type.
 */
public class GraphControlDependencyCalculator {
    ...
    private <T extends IGraphNode<T>> void propagatePostDominator(Map<T, Set<T>> postDominanceMap,
        T node,
        Boolean isChange, Set<T> visitedBlocks) {
        visitedBlocks.add(node);

        Set<T> intersetion = findIntersetedPostDominator(node.getChildren(),
            postDominanceMap);
    ...
    private <T extends IGraphNode<T>> Map<T, List<T>> constructControlDependency(IGraph<T> graph,
        ...
        for (T branchNode : branchNodes) {
            computeControlDependentees(branchNode, branchNode.getChildren(), graph,
                postDominanceMap, controlDependencyMap);
        }
        return controlDependencyMap;
    }

    private <T extends IGraphNode<T>> void computeControlDependentees(T branchNode,
        List<T> children, IGraph<T> graph,
        ...
        for (T child : CollectionUtils.nullToEmpty(children)) {
            if (!dependentees.contains(child)) {
                if (!postDominanceMap.get(child).contains(branchNode)) {
                    ...
                    computeControlDependentees(branchNode, child.getChildren(), graph,
                        postDominanceMap, controlDependencyMap);
                }
            }
        }
