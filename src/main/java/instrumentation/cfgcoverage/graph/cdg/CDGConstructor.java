package instrumentation.cfgcoverage.graph.cdg;

import instrumentation.cfgcoverage.graph.CoverageSFNode;
import instrumentation.cfgcoverage.graph.CoverageSFlowGraph;
import instrumentation.cfgcoverage.graph.GraphControlDependencyCalculator;
...
public class CDGConstructor {
    ...
    private final GraphControlDependencyCalculator controlDependencyCalcul = new GraphControlDependencyCalculator();
        ...
        while (!visitStack.empty()) {
            ...
            if (!visited.contains(visitingNode.getCvgIdx())) {
                ...
                if (visitingNode.isConditionalNode()) {
                    curDominatorNode = new CDGNodeHolder(cdgNodeMap, visitingNode,
                ...
                for (CoverageSFNode child : CollectionUtils.nullToEmpty(
                    visitingNode.getChildren())) {
                    visitStack.push(child);
                }
            } else {
                if (!condNodeStack.isEmpty()
                    && condNodeStack.peek().dominator.getCfgNode() == visitingNode) {
                    condNodeStack.pop();
                }
                if (condNodeStack.isEmpty()) {
