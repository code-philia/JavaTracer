package org.cophi.javatracer.codeanalysis.runtime;
...
public abstract class Executor {
    ...
    private static String[] defaultLibExcludes = {
        ...
        "org.xml.*",
        "sunw.*",
        "org.junit.*", "junit.*", "junit.framework.*", "org.hamcrest.*", "org.hamcrest.core.*",
...
//    /**
//     * This method derive more detailed libExcludes with libIncludes, for example, when libExcludes
//     * has a pattern as java.* while libIncludes has a pattern as java.util.*, then the method can
//     * split java.* into a list including java.awt.*, java.lang.*, ..., except java.util.*.
//     *
//     * @return
//     */
...
//        expandedExcludes = FilterUtils.deriveLibExcludePatterns(pkgsContainer, excludedLibs,
//            includedLibs);
//        return StringUtils.sortAlphanumericStrings(new ArrayList<>(expandedExcludes))
...
//    }
    ...
    protected void addStepWatch(EventRequestManager erm, ThreadReference threadReference) {
        StepRequest stepRequest = erm.createStepRequest(threadReference, StepRequest.STEP_LINE,
...
//            }
//
//            CompilationUnit parentCU = JavaUtil.findCompilationUnitInProject(fullName,
//                appClassPath);
...
//				&& !thread.name().contains("RMI TCP")
    ...
    /**
     * When analyzing a JUnit test case, we will not record the trace, until the target test case is
     * loaded. In order to see whether the target test case is loaded, there is specific JUnit
     * method is called. This method returns whether such the specific JUnit method has been
     * called.
     *
     * @param declareType
     * @param methodName
    ...
    protected boolean isTagJUnitCall(String declareType, String methodName) {
        return
            (declareType.contains("junit.framework.TestResult") && methodName.equals("startTest"))
                ||
                (declareType.contains("org.junit.internal.runners.model.ReflectiveCallable")
                    && methodName.equals("run"));
    }

//    protected Value retrieveComplicatedExpressionValue(String expr, VirtualMachine vm,
