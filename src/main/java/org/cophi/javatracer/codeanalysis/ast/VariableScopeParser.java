...
public class VariableScopeParser {
    ...
    public LocalVariableScope parseMethodScope(String typeName, final int lineNumber,
        final String variableName, ProjectConfig projectConfig) {
        final CompilationUnit cu = JavaUtil.findCompilationUnitInProject(typeName, projectConfig);
        ...
        if (!variableScopeList.isEmpty()) {
            ...
            for (LocalVariableScope scope : this.variableScopeList) {
                if (scope.getStartLine() > minScope.getStartLine()
                    && scope.getEndLine() < minScope.getEndLine()) {
                    minScope = scope;
                }
            }
            return minScope;
