...
import org.cophi.javatracer.model.variables.ConstantVar;
...
public class SingleLineByteCodeVisitor extends ByteCodeVisitor {
    ...
    public SingleLineByteCodeVisitor(int lineNumber, String className,
        ProjectConfig appJavaClassPath) {
        super();
    ...
    public void visitMethod(Method method) {
        ...
        if (isMethodContainLineNumber(method, lineNumber)) {
            ...
            for (InstructionHandle insHandle : insHandles) {
                VarOp varOp = parseReadWrittenVariable(insHandle, method, projectConfig);
    ...
    protected List<InstructionHandle> findPreviousInstructions(int lineNumber, long offset,
        Code code) {
    ...
    protected VarOp parseReadWrittenVariable(InstructionHandle insHandle, Method method,
        ProjectConfig appJavaClassPath) {
        Code code = method.getCode();
        ...
        if (insHandle.getInstruction() instanceof FieldInstruction) {
            ...
            List<InstructionHandle> previousInstructions = findPreviousInstructions(
                insHandle.getPosition(), code);
...
//				System.currentTimeMillis();
     ...
     * name. The basic idea is that, given an array instruction AINS such as aaload or aastore, we
     * find its nearest previous instruction handling an array variable. The name of handled array
     * variable is assumed to be the name of the given array. However, we should take care that the
     * instructions popping an array out of frame stack. If we detect k such instructions, the name
     * of variable handled by k-th instruction from AINS should be the name we need. We record k in
     * a local variable
    ...
    private Variable parseArrayName(Method method, List<InstructionHandle> previousInstructions) {
        ...
        for (int i = previousInstructions.size() - 1; i >= 0; i--) {
            ...
            if (ins instanceof FieldInstruction) {
                ...
                LocalVariable localVariable = code.getLocalVariableTable()
                    .getLocalVariable(localVarIndex, insHandle.getPosition());
                ...
                if (type.contains("[")) {
                    ...
                    int varIndex = lIns.getIndex();
                    LocalVariable lVar = method.getLocalVariableTable()
                        .getLocalVariable(varIndex, insHandle.getPosition());
                    if (lVar != null) {
    ...
    private void parseReturnVariable(InstructionList list, Method method) {
        for (InstructionHandle handle : list) {
            if (handle.getInstruction() instanceof ReturnInstruction) {
                List<InstructionHandle> previousInstructions = findPreviousInstructions(lineNumber,
                    handle.getPosition(), method.getCode());
    ...
    private Variable parseReturnVariableName(Method method,
        List<InstructionHandle> previousInstructions) {
        for (int i = previousInstructions.size() - 1; i >= 0; i--) {
            InstructionHandle insHandle = previousInstructions.get(i);
            VarOp varOp = parseReadWrittenVariable(insHandle, method, projectConfig);
