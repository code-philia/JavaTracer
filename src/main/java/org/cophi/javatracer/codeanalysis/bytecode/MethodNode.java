...
public class MethodNode {
    ...
    private void findAllCallers(Map<MethodNode, List<InstructionHandle>> map,
    ...
    private List<InstructionHandle> findFieldDefinition(FieldVar field) {
        ...
        for (InstructionHandle handle : list.getInstructionHandles()) {
            ...
            if (instruction instanceof PUTFIELD) {
                ...
                if (field.getName().equals(fieldName) && field.getDeclaringType()
                    .equals(className)) {
                    hList.add(handle);
                }
            }
            ...
            if (instruction instanceof StoreInstruction) {
                ...
                if (method.getLocalVariableTable() != null) {
                    LocalVariable lVar = method.getLocalVariableTable()
                        .getLocalVariable(sIns.getIndex(), handle.getPosition());
                    if (lVar != null) {
                        String typeName = SignatureUtils.signatureToName(lVar.getSignature());
                        if (var.getType().equals(typeName) && lVar.getName()
                            .equals(var.getName())) {
                            hList.add(handle);
                        }
