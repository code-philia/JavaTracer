...
import org.cophi.javatracer.model.ControlScope;
import org.cophi.javatracer.model.Scope;
...
public class Trace {
    ...
    private final VariableDefinitions variableDefs = new VariableDefinitions();
...
//	}
    ...
    private void fillInControlScope() {
        ...
        for (BreakPoint bkp : breakpointMap.keySet()) {
            ...
            CollectionUtils.getSetInitIfEmpty(classMethodMap, bkp.getClassCanonicalName())
                .add(bkp.getMethodSign());
        }
    ...
    private List<ClassLocation> findControlledLines(List<CFGNode> controlDependentees,
        Method method,
        BreakPoint breakPoint) {
        List<ClassLocation> controlLines = new ArrayList<>();
        for (CFGNode node : controlDependentees) {
            int line = method.getLineNumberTable()
                .getSourceLine(node.getInstructionHandle().getPosition());
            ClassLocation location = new ClassLocation(breakPoint.getClassCanonicalName(),
                breakPoint.getMethodSign(), line);
    ...
    private List<InstructionHandle> findCorrepondingIns(BreakPoint breakPoint, CFG cfg,
        Method method) {
    ...
    private ControlScope parseControlScope(BreakPoint breakPoint, CFG cfg) {
        ...
        List<InstructionHandle> correspondingList = findCorrepondingIns(breakPoint, cfg,
            cfg.getMethod());
        ...
        for (InstructionHandle ins : correspondingList) {
            ...
            List<ClassLocation> controlScope0 = findControlledLines(
                cfgNode.getControlDependentees(), cfg.getMethod(), breakPoint);
            ranges.addAll(controlScope0);
        }

        ClassLocation own = new ClassLocation(breakPoint.getClassCanonicalName(),
            breakPoint.getMethodSign(), breakPoint.getLineNumber());
