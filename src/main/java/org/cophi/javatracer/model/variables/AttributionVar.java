...
/**
 * This class represents the attribution relation among variables. If a variable is considered
 * wrong, it is possible caused by other wrong variable. If it is the case, we say that the
 * incorrectness of the variable is attributed to other wrong variable. Based on such relation, we
 * are able to build a graph in which each node is a variable and each edge is an attribution
 * relation. Note that, given that each variable will be assigned only once (see SSA form), thus,
