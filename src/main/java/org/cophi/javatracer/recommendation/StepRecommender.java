...
import org.cophi.javatracer.model.Cause;
import org.cophi.javatracer.model.trace.PathInstance;
...
import org.cophi.javatracer.model.trace.TraceNodeOrderComparator;
...
//		}
    ...
    /**
     * Find the variable causing the jump of label path of the <code>pattern</code>, noted as
     * <code>var</code>, then try finding the dominator of the <code>oldSusiciousNode</code> by
     * following the same dominance chain with regard to <code>var</code>. The dominator of
     * <code>oldSusiciousNode</code> on <code>var</code> is the new suspicious node.
     * <br><br>
    ...
    public TraceNode recommendNode(Trace trace, TraceNode currentNode, UserFeedback userFeedback) {
        ...
        if (feedbackType.equals(UserFeedback.UNCLEAR)) {
            ...
            if (state == DebugState.SLICING || state == DebugState.SKIP
                || state == DebugState.BINARY_SEARCH || state == DebugState.DETAIL_INSPECT) {
            ...
            (feedbackType.equals(UserFeedback.WRONG_VARIABLE_VALUE) || feedbackType.equals(
                UserFeedback.WRONG_PATH))) {
    ...
    private TraceNode findClearerNodeInBetween(Trace trace, TraceNode currentNode,
        TraceNode earliestVisitedUnclearNode) {
        ...
        if (latestWrongNode != null) {
            ...
            while (iter.hasNext()) {
                ...
                boolean shouldRemove = dominator.getOrder() < currentNode.getOrder() ||
                    ...
                    dominator.getAbstractionLevel()
                        > earliestVisitedUnclearNode.getAbstractionLevel();
                if (shouldRemove) {
                    iter.remove();
                }
            }
    ...
    private TraceNode handleBinarySearchState(Trace trace, TraceNode currentNode,
...
//			this.range.update();
            ...
            if (startNode != null) {
                ...
                PotentialCorrectPattern pattern = Settings.potentialCorrectPatterns.getPattern(
                    fakePath);
                if (pattern != null) {
                    PathInstance labelPath = pattern.getLabelInstance();
                    Variable causingVariable = labelPath.findCausingVar();

                    Variable compatiableVariable = findCompatibleReadVariable(causingVariable,
                        currentNode);
...
//						this.range.update();
                        ...
                        suspiciousNode = handleSimpleInferenceState(trace, currentNode,
                            userFeedback);
                    }
                } else {
                    System.err.println("error in binary search for " + fakePath.getLineTrace()
                        + ", cannot find corresponding pattern");
                }
            } else {
...
//		TraceNode suspiciousNode = trace.findSuspiciousControlDominator(currentNode, feedback);
    ...
    private TraceNode handleDataSlicing(Trace trace, TraceNode currentNode,
        UserFeedback userFeedback) {
...
//		}
    ...
    private TraceNode handleDetailInspectingState(Trace trace, TraceNode currentNode,
        UserFeedback userFeedBack) {
    ...
    /**
     * Given the state is simple inference, which means the state is caused by either simple data
     * dominance or simple control dominance, this method interpret the user feedback into events to
     ...
     * @return
     */
    private TraceNode handleSimpleInferenceState(Trace trace, TraceNode currentNode,
        UserFeedback userFeedBack) {
        ...
        if (userFeedBack.getFeedbackType().equals(UserFeedback.WRONG_PATH)) {
            ...
            InspectingRange inspectingRange = new InspectingRange(currentNode,
                latestCause.getBuggyNode());
            this.detailInspector.setInspectingRange(inspectingRange);

            TraceNode recommendedNode = handleDetailInspectingState(trace, currentNode,
                userFeedBack);
    ...
    private TraceNode handleSkipState(Trace trace, TraceNode currentNode,
        UserFeedback userFeedback) {
        ...
        if (!suspiciousNode.hasChecked()) {
            ...
            if (suspiciousNode.isAllReadWrittenVarCorrect(false)) {
                ...
                if (pattern != null) {
                    ...
                    Variable compatibaleVariable = findCompatibleReadVariable(causingVariable,
                        suspiciousNode);
