...
//		VarValue specificVar = findSpecificWrongVar(start, end);
        ...
        Map<MethodNode, List<InstructionHandle>> seeds = seedFinder.findSeedStatemets(wrongValue,
            start, end);
        if (seeds == null) {
            return new ArrayList<>();
        }
        ...
        List<BreakPoint> controlPoints = controlFinder.findSeedControlDominators(
            trace.allLocations(), seeds);
