...
public class BugInferer {
    ...
    public Bug infer(TraceNode currentNode, StepRecommender recommender) {
        ...
        //Branch bug
        if (startNode == null) {
            startNode = findIterationHead(currentNode, Settings.potentialCorrectPatterns,
                recommender);
            if (startNode != null) {
                PathInstance path = new PathInstance(startNode, currentNode);
                List<PathInstance> labelPaths = Settings.potentialCorrectPatterns.findSimilarIterationPath(
                    path);
    ...
    private TraceNode findIterationHead(TraceNode currentNode,
        ...
        List<PotentialCorrectPattern> patterns = potentialCorrectPatterns.findPatternsHavingSameEndNodeWith(
            line);
    ...
    private List<TraceNode> pathDiff(PathInstance path, PathInstance labelPath) {
        ...
        Object[] commonLines = JavaTracerUtils.generateCommonNodeList(lines, labelLines);
        ...
        while (startNode.getOrder() <= path.getEndNode().getOrder()) {
            SourceLine line = path.new SourceLine(startNode.getClassCanonicalName(),
                startNode.getLineNumber(),
