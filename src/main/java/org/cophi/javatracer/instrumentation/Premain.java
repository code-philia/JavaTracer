...
import org.cophi.javatracer.instrumentation.instr.TestRunnerTranformer;
import java.io.File;
...
//										"slf4j-api-1.7.12.jar"
    ...
    private static List<JarFile> getJarFiles(String... jarNames) throws Exception {
        ...
        for (String jarName : jarNames) {
            ...
            if ((!isUptodate && CollectionUtils.existIn(file.getName(),
                INSTRUMENTATION_STANTDALONE_JAR, SAV_JAR)) || !file.exists()) {
                try {
    ...
    public static boolean extractJar(String jarResourcePath, String filePath) throws IOException {
        final InputStream inputJarStream = Premain.class.getClassLoader()
            .getResourceAsStream(jarResourcePath);
