...
public class LocalVariableSupporter {
    ...
    public static void fillUpVariableTable(MethodGen methodGen, Method method,
        ...
        for (InstructionHandle insnHandler : methodGen.getInstructionList()) {
            if (!(insnHandler.getInstruction() instanceof LocalVariableInstruction insn)) {
                continue;
            }
        ...
        for (LocalVariableInfo localVar : localVariables) {
            methodGen.addLocalVariable(VAR_ALIAS_NAME_PREFIX + count++, localVar.type,
                localVar.index, localVar.startPos,
                localVar.endPos);
        }
    }
