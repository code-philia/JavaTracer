 ...
 * @author LLT
 * <p>
 * With the approach of using a TestRunner to execute the testcase, in order to trace the flow of
 * the process of executing a testcase, we instrument the TestRunner to call our Agent at particular
 * and provide some information for logging.
 * <p>
 * Methods for instrumentation in TestRunner must + start with a "$" and + not be an abstract and
 * have empty body
 * <p>
...
public class TestRunnerTranformer extends AbstractTransformer implements ClassFileTransformer {
    ...
    private byte[] instrument(String classFName, byte[] classfileBuffer)
        ...
        for (Method method : jc.getMethods()) {
            ...
            if ("$exitProgram".equals(method.getName())) {
                agentMethodIdx = constPool.addMethodref(Agent.class.getName().replace(".", "/"),
                    "_exitProgram",
                    "(Ljava/lang/String;)V");
                paramSize = 1;
                ...
                agentMethodIdx = constPool.addMethodref(
                    CoverageAgent.class.getName().replace(".", "/"), "_storeCoverage",
                    "(Ljava/io/OutputStream;Ljava/lang/Boolean;)V");
                paramSize = 2;
            }
            if (agentMethodIdx >= 0) {
                instrumentDelegateMethod(method, classFName, constPool, agentMethodIdx, classGen,
                    paramSize);
            }
    ...
    private void instrumentDelegateMethod(Method method, String classFName,
