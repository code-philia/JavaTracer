...
public class MethodSplitter {
    ...
    /**
     * LLT: methodGen in this case is over-long one, so be careful when working on the object since
     * some methods will throw exception when being triggered (methods which requires dumping out
     * bytecode). for ex: getByteCode(), getMethod()
     */
    ...
    private MethodGen createMethod(Type returnType, Type[] arg_types, InstructionList instrnList) {
        ...
        MethodGen methodgen = new MethodGen(access_flags, returnType, arg_types, arg_name,
