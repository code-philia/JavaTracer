...
import org.cophi.javatracer.instrumentation.instr.instruction.info.ArrayInstructionInfo;
import org.cophi.javatracer.instrumentation.instr.instruction.info.EntryPoint;
import org.cophi.javatracer.instrumentation.instr.instruction.info.FieldInstructionInfo;
import org.cophi.javatracer.instrumentation.instr.instruction.info.LineInstructionInfo;
import org.cophi.javatracer.instrumentation.instr.instruction.info.LocalVarInstructionInfo;
import org.cophi.javatracer.instrumentation.instr.instruction.info.RWInstructionInfo;
...
import org.cophi.javatracer.instrumentation.runtime.TraceUtils;
...
public class TraceInstrumenter extends AbstractInstrumenter {
        ...
        for (Method method : jc.getMethods()) {
            ...
            try {
                ...
                boolean isMainMethod =
                    entry && entryPoint.matchMethod(method.getName(), method.getSignature());

                boolean isEntry = method.getName().equals("run") && isThread(jc);

                GeneratedMethods generatedMethods = runMethodInstrumentation(classGen, constPool,
                    methodGen,
    ...
    private GeneratedMethods runMethodInstrumentation(ClassGen classGen, ConstantPoolGen constPool,
        MethodGen methodGen, Method method,
        boolean isAppClass, boolean isMainMethod, boolean isEntry) {
        String methodFullName = MethodUtils.getMethodFullName(classGen.getClassName(), method);
        boolean changed = instrumentMethod(classGen, constPool, methodGen, method, isAppClass,
            isMainMethod, isEntry);
    ...
    protected boolean instrumentMethod(ClassGen classGen, ConstantPoolGen constPool,
        MethodGen methodGen, Method method,
        boolean isAppClass, boolean isMainMethod, boolean isEntry) {
        if (!userFilters.isInstrumentable(classGen.getClassName(), method,
        ...
        List<LineInstructionInfo> lineInsnInfos = LineInstructionInfo.buildLineInstructionInfos(
            classGen, constPool,
        ...
        LocalVariableGen tracerVar = methodGen.addLocalVariable(TRACER_VAR_NAME,
            Type.getType(IExecutionTracer.class),
...
//			instructionTable.put(bp.hashCode(), lineInfo.getSerializable());
            ...
            injectCodeTracerHitLine(insnList, constPool, tracerVar, lineInfo.getLine(),
                lineInfo.getLineNumberInsn(),
                classNameVar, methodSigVar, lineInfo.hasExceptionTarget(),
...
//			}
            for (RWInstructionInfo rwInsnInfo : rwInsns) {
                ...
                if (rwInsnInfo instanceof FieldInstructionInfo) {
                    newInsns = getInjectCodeTracerRWriteField(constPool, tracerVar,
                        (FieldInstructionInfo) rwInsnInfo, classNameVar, methodSigVar);
            ...
            for (InstructionHandle insn : lineInfo.getInvokeInstructions()) {
                injectCodeTracerInvokeMethod(methodGen, insnList, constPool, instructionFactory,
                    tracerVar, insn, line,
                    classNameVar, methodSigVar, isAppClass);
            }
            /* instrument Return instructions */
            for (InstructionHandle insn : lineInfo.getReturnInsns()) {
                injectCodeTracerReturn(insnList, constPool, tracerVar, insn, line, classNameVar,
                    methodSigVar);
            }
            ...
            for (InstructionHandle exitInsHandle : lineInfo.getExitInsns()) {
                injectCodeTracerExit(exitInsHandle, insnList, constPool, tracerVar, line,
                    classNameVar, methodSigVar, isMainMethod, isEntry);
            }
    ...
    private void injectCodeTracerExit(InstructionHandle exitInsHandle, InstructionList insnList,
        ...
        if (isMainMethod || isEntry) {
            int index = constPool.addInterfaceMethodref(Agent.class.getName().replace(".", "/"),
                "_exitProgram",
                "(Ljava/lang/String;)V");
            newInsns.append(new ALOAD(classNameVar.getIndex()));
            newInsns.append(new ALOAD(methodSigVar.getIndex()));
    ...
    private void injectCodeTracerReturn(InstructionList insnList, ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        InstructionHandle insnHandler, int line, LocalVariableGen classNameVar,
        ...
        if (insn instanceof RETURN) {
            ...
            if (insnHandler.getPrev().getInstruction() instanceof ACONST_NULL) {
                ...
                if (type instanceof BasicType) {
                    newInsns.append(
                        new INVOKESTATIC(
                            basicTypeSupporter.getValueOfMethodIdx((BasicType) type, constPool)));
                }
            }
    ...
    private void injectCodeTracerInvokeMethod(MethodGen methodGen, InstructionList insnList,
        ConstantPoolGen constPool,
        InstructionFactory instructionFactory, LocalVariableGen tracerVar,
         ...
         */
        LocalVariableGen argObjsVar = addTempVar(methodGen, new ArrayType(Type.OBJECT, 1),
            insnHandler);
        ...
        for (int i = (argTypes.length - 1); i >= 0; i--) {
            ...
            if (argType instanceof BasicType) {
                newInsns.append(
                    new INVOKESTATIC(
                        basicTypeSupporter.getValueOfMethodIdx((BasicType) argType, constPool)));
            }
        ...
        for (int i = 0; i < argTypes.length; i++) {
            ...
            if (argType instanceof BasicType) {
                newInsns.append(instructionFactory
                    .createCheckCast(
                        basicTypeSupporter.getCorrespondingPrimitiveType((BasicType) argType)));
                newInsns.append(new INVOKEVIRTUAL(
                    basicTypeSupporter.getToPrimitiveValueMethodIdx((BasicType) argType,
                        constPool)));
            }
        }
        ...
        /* after_invoke */
        if (isAppClass) {
            injectCodeAfterInvoke(insnList, constPool, tracerVar, insnHandler, line, classNameVar,
                methodSigVar,
                isInvokeStatic, returnType, mSig);
        }
    }

    private void injectCodeAfterInvoke(InstructionList insnList, ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        InstructionHandle insnHandler, int line, LocalVariableGen classNameVar,
        LocalVariableGen methodSigVar,
...
//					|| !(insnHandler.getNext().getInstruction() instanceof POP));
        ...
        if (isInvokeStatic) {
            ...
            if (Type.VOID.equals(returnType)) {
                ...
                basicTypeSupporter.appendObjectConvertInstruction(returnType, newInsns,
                    constPool); // returnValue*, objectRef, returnValue
                newInsns.append(new ALOAD(
                    tracerVar.getIndex())); // [returnValue*], objectRef, returnValue, $trace
                newInsns.append(
                    new DUP_X2()); // [returnValue*], $trace, objectRef, returnValue, $trace
    ...
    private InstructionList getInjectCodeTracerRWriteField(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
    ...
    private InstructionList getInjectCodePutField(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        FieldInstructionInfo info, LocalVariableGen classNameVar, LocalVariableGen methodSigVar) {
        InstructionList newInsns = new InstructionList();
        /*
        ...
        if (fieldType instanceof BasicType) {
            newInsns.append(new INVOKESTATIC(
                basicTypeSupporter.getValueOfMethodIdx((BasicType) fieldType, constPool)));
        }
     ...
     * @param classNameVar
     */
    private InstructionList getInjectCodePutStatic(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        FieldInstructionInfo info, LocalVariableGen classNameVar, LocalVariableGen methodSigVar) {
        InstructionList newInsns = new InstructionList();
        if (info.isNextToAconstNull()) {
    ...
    protected void appendTracerMethodInvoke(InstructionList newInsns, TracerMethods method,
        ConstantPoolGen constPool) {
    ...
    private InstructionList getInjectCodeGetField(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        FieldInstructionInfo info, LocalVariableGen classNameVar, LocalVariableGen methodSigVar) {
        InstructionList newInsns = new InstructionList();
        ...
        newInsns.append(
            new PUSH(constPool, info.getFieldName())); // [val*, obj], tracer, obj, val, fieldName
        newInsns.append(new PUSH(constPool,
            info.getFieldType())); // [val*, obj], tracer, obj, val, fieldName, fieldTypeSignature
        newInsns.append(new PUSH(constPool,
            info.getLine())); // [val*, obj], tracer, obj, val, fieldName, fieldTypeSignature, line
        newInsns.append(new ALOAD(classNameVar.getIndex()));
        newInsns.append(new ALOAD(methodSigVar.getIndex()));
    ...
    private InstructionList getInjectCodeGetStatic(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        FieldInstructionInfo info, LocalVariableGen classNameVar, LocalVariableGen methodSigVar) {
        InstructionList newInsns = new InstructionList();
    ...
    private InstructionList getInjectCodeTracerRWLocalVar(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        LocalVarInstructionInfo insnInfo, LocalVariableGen classNameVar,
        LocalVariableGen methodSigVar) {
...
//		}
        ...
        /* invoke tracer */
        if (insnInfo.isStoreInstruction() && insnInfo.getInstructionHandler().getPrev()
    ...
    private InstructionList getInjectCodeTracerIINC(ConstantPoolGen constPool,
        LocalVariableGen tracerVar,
        LocalVarInstructionInfo insnInfo, LocalVariableGen classNameVar,
        LocalVariableGen methodSigVar, boolean isStatic) {
    ...
    private InstructionList getInjectCodeTracerRWriteArray(MethodGen methodGen,
        ConstantPoolGen constPool,
        ...
        if (info.getElementType() instanceof BasicType) {
            newInsns.append(
                new INVOKESTATIC(
                    basicTypeSupporter.getValueOfMethodIdx((BasicType) info.getElementType(),
                        constPool)));
        }
    ...
    private LocalVariableGen addTempVar(MethodGen methodGen, Type type,
        InstructionHandle insnHandler) {
        return methodGen.addLocalVariable(nextTempVarName(), type, insnHandler,
            insnHandler.getNext());
    }
    ...
    protected void injectCodeTracerHitLine(InstructionList insnList, ConstantPoolGen constPool,
        ...
        LineInstructionInfo lineInfo) {
        TracerMethods tracerMethod =
            isExceptionTarget ? TracerMethods.HIT_EXEPTION_TARGET : TracerMethods.HIT_LINE;
        InstructionList newInsns = new InstructionList();
        newInsns.append(new ALOAD(tracerVar.getIndex()));
        newInsns.append(new PUSH(constPool, line));
        newInsns.append(new ALOAD(classNameVar.getIndex()));
    ...
    protected LocalVariableGen injectCodeInitTracer(MethodGen methodGen, ConstantPoolGen constPool,
        int methodStartLine,
        int methodEndLine, boolean isAppClass, LocalVariableGen classNameVar,
        LocalVariableGen methodSigVar,
        ...
        newInsns.append(new PUSH(constPool,
            methodStartLine));    // startTracing, className, String methodSig, int methodStartLine
        newInsns.append(new PUSH(constPool,
            methodEndLine)); // startTracing, className, String methodSig, int methodStartLine, methodEndLine

        String[] argList = getArgumentNames(methodGen);
        newInsns.append(new PUSH(constPool, TraceUtils.encodeArgNames(argList)));
        newInsns.append(
            new PUSH(constPool, TraceUtils.encodeArgTypes(methodGen.getArgumentTypes())));
