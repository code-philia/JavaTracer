package org.cophi.javatracer.instrumentation.output;
...
import org.cophi.javatracer.model.SourceScope;
...
import org.cophi.javatracer.utils.BreakpointUtils;
...
public class TraceOutputWriter extends OutputWriter {
    ...
    public void writeTrace(Trace trace, String projectName, String projectVersion,
        String launchClass,
    ...
    private void writeFilterInfo(List<String> libClasses, boolean isInclusive) throws IOException {
        if (libClasses.size() > 300 && (traceExecFolder != null)) {
            ...
            String fileName =
                filterFilePrefix + (isInclusive ? "_includes.info" : "_excludes.info");
    ...
    private Map<String, Integer> writeLocations(Trace trace) throws IOException {
        ...
        for (String className : locationMap.keySet()) {
            ...
            for (BreakPoint bkp : bkps) {
                if (i == 0) {
                    writeString(
                        bkp.getDeclaringCompilationUnitName()); // DeclaringCompilationUnitName
                    i++;
                }
