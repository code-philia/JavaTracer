...
public class StringUtils {
  ...
  public static String toJoinStrCode(List<?> allVars) {
    ...
    for (int i = 0; i < allVars.size(); i++) {
      ...
      if (val == null) {
        ...
        varSb.append("\"").append(val).append("\"");
      }
      if (i != allVars.size() - 1) {
        varSb.append(", ");
      }
  ...
  /**
   * @param packageName the package name as specified in the package declaration (i.e. a
   *     dot-separated name)
   * @param simpleTypeName the simple name of the type
   * @param enclosingTypeNames if the type is a member type, the simple names of the enclosing types
   *     from the outer-most to the direct parent of the type (for example, if the class is
   *     x.y.A$B$C then the enclosing types are [A, B]. This is an empty array if the type is a
   *     top-level type.
   */
  public static String dotJoinStr(
      char[] packageName, char[][] enclosingTypeNames, char[] simpleTypeName) {
    Object[] enclosing = new Object[enclosingTypeNames.length];
    for (int i = 0; i < enclosingTypeNames.length; i++) {
      enclosing[i] = new String(enclosingTypeNames[i]);
    }
    return dotJoin(new String(packageName), dotJoin(enclosing), new String(simpleTypeName));
  }
